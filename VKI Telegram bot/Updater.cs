...
namespace VKI_Telegram_bot
{
    public static class Updater
    {
        ...
        public static async Task Start(TelegramBotClient _tb, CancellationToken ctn)
        {
            ...
            try
            {
                while (!ctn.IsCancellationRequested)
                {
                    ...
                    await PDFPUpdate(iertification, "Промежуточная аттестация обновилась");
                    await PDFPUpdate(sgroup, "Списки групп обновились");
                    await PDFPUpdate(timetable, "Расписание обновилось");
                    await ScheduleUpdate(schedule, "Расписание звонков обновилось");
        ...
        public static async Task ScheduleUpdate(Schedule parser, string messege)
        {
        ...
        public static async Task PDFPUpdate(PDFParser parser, string messege)
        {
            await parser.UpdateAsync();
            using (VKITGBContext db = new VKITGBContext())
            {
